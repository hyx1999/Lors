...
class SpLoraFn(Function):
    ...
    def forward(
        ...
        ctx.save_for_backward(x, weight, mask, lora_A, lora_B)
        ctx.scaling = scaling
    ...
    def backward(ctx, 
        ...
        x, weight, mask, lora_A, lora_B = ctx.saved_tensors
        scaling = ctx.scaling
        ...
        return grad_x, grad_weight, grad_bias, None, grad_lora_A, grad_lora_B, None, None


class SpLoraMaskLinear(Linear):
    ...
    def __init__(self, 
        ...
        self.mask = nn.Parameter(torch.empty(out_features, in_features, dtype=torch.bool), requires_grad=False)
    ...
    @torch.no_grad()
    def set_mask(self):
        self.mask.copy_(self.weight != 0)

    @torch.no_grad()
    def merge_adapter(self):
        if hasattr(self, "lora_A"):
            self.weight.data += (self.lora_B @ self.lora_A) * self.mask
            delattr(self, "mask")
            delattr(self, "lora_A")
            delattr(self, "lora_B")
